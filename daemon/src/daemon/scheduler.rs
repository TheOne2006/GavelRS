...
use gavel_core::utils::models::{TaskState, TaskMeta, ResourceLimit, MemoryRequirementType}; // Import necessary models, ResourceLimit, MemoryRequirementType
...
use std::process::Stdio; // For redirecting output
...
fn is_gpu_qualifying_for_queue(
    gpu_id: u32,
    gpu_stat: &GpuStats,
    limit: &ResourceLimit,
) -> bool {
    ...
    let memory_ok = match limit.memory_requirement_type {
        ...
        MemoryRequirementType::AbsoluteMb => {
            free_memory_mb >= limit.memory_requirement_value
        }
...
async fn schedule_tasks(state: &DaemonState) -> Result<()> {
    ...
    let all_tasks = state.get_all_tasks().await.into_iter().map(|t| (t.id, t)).collect::<HashMap<_, _>>();
    ...
    'queue_loop: for queue_meta in queues {
        ...
        for task_id in &current_queue.running_task_ids {
            if let Some(task) = all_tasks.get(task_id) {
                 ...
                 warn!("Running task ID {} in queue {} not found in global map. Inconsistency?", task_id, current_queue.name);
            }
        }
        ...
        for task in waiting_tasks_meta {
            ...
            if can_run {
                info!("Attempting to start task {} (ID: {}) from queue {}", task.name, task.id, current_queue.name);

                // 9. 更新任务状态并分配 GPU (atomically)
                // Pass the GPUs intended for assignment
                match state.update_task_state(task.id, TaskState::Running, Some(gpus_to_assign.clone())).await {
                    Ok(_) => {
                        ...
                        if !gpus_to_assign.is_empty() {
                             let assigned_set: HashSet<u32> = gpus_to_assign.iter().map(|&id| id as u32).collect();
                        ...
                        if let Some(updated_task) = state.get_task(task.id).await {
                             ...
                             if let Err(e) = launch_task_process(&state, updated_task).await {
                                error!("Failed to launch task process for {} (ID: {}): {}", task.name, task.id, e);
                                // Attempt to revert state or handle error more robustly
                                if let Err(revert_err) = state.update_task_state(task.id, TaskState::Waiting, None).await { // Revert to Waiting, clear GPUs
                                    error!("Failed to revert task state for {} (ID: {}): {}", task.name, task.id, revert_err);
                                }
                             } else {
                             ...
                             if let Err(revert_err) = state.update_task_state(task.id, TaskState::Waiting, None).await {
                                error!("Failed to revert task state for {} (ID: {}): {}", task.name, task.id, revert_err);
                             }
                        }
...
async fn launch_task_process(state: &DaemonState, task: TaskMeta) -> Result<()> {
    info!("Launching task process for \'{}\' (ID: {}). CMD: \'{}\'. Log: \'{}\'", task.name, task.id, task.cmd, task.log_path);
    ...
    if !task.gpu_ids.is_empty() {
        let cuda_visible_devices = task.gpu_ids.iter().map(|id| id.to_string()).collect::<Vec<_>>().join(",");
        info!("Task {} (ID: {}): Setting CUDA_VISIBLE_DEVICES={}", task.name, task.id, cuda_visible_devices);
        command.env("CUDA_VISIBLE_DEVICES", cuda_visible_devices);
    } else {
        info!("Task {} (ID: {}): No GPUs assigned, not setting CUDA_VISIBLE_DEVICES.", task.name, task.id);
    }
    ...
    */
    ...
    let mut child = match command.spawn() {
        ...
        Err(e) => {
            ...
            if let Err(update_err) = state.update_task_state(task.id, TaskState::Finished, None).await {
    ...
    tokio::spawn(async move {
        info!("Monitoring task \'{}\' (ID: {}) with PID {:?} for completion.", task_name_for_monitor, task_id_for_monitor, pid);
        let exit_status_result = child.wait().await;

        let final_state = match exit_status_result {
            Ok(status) => {
                if status.success() {
                    info!("Task \'{}\' (ID: {}) with PID {:?} finished successfully. Exit status: {}", task_name_for_monitor, task_id_for_monitor, pid, status);
                    TaskState::Finished
                } else {
                    warn!("Task \'{}\' (ID: {}) with PID {:?} failed. Exit status: {}", task_name_for_monitor, task_id_for_monitor, pid, status);
                    TaskState::Finished
                }
            }
            Err(e) => {
                error!("Error waiting for task \'{}\' (ID: {}) with PID {:?} to complete: {}", task_name_for_monitor, task_id_for_monitor, pid, e);
                TaskState::Finished // Assume failed if waiting errored
            }
        };

        // Update task state in DaemonState
        if let Err(e) = state_clone_for_monitor.update_task_state(task_id_for_monitor, final_state, None).await {
            error!("Failed to update final state for task \'{}\' (ID: {}): {}", task_name_for_monitor, task_id_for_monitor, e);
        }

        // Clear PID in DaemonState
        if let Err(e) = state_clone_for_monitor.set_task_pid(task_id_for_monitor, None).await {
            error!("Failed to clear PID for task \'{}\' (ID: {}): {}", task_name_for_monitor, task_id_for_monitor, e);
        }
        info!("Monitoring finished for task \'{}\' (ID: {}).", task_name_for_monitor, task_id_for_monitor);
    });
...
async fn update_tasks(state: &DaemonState) -> Result<()> {
        ...
        if task.state == TaskState::Running && task.pid.is_some() {
            ...
            match tokio::fs::metadata(format!("/proc/{}/status", pid_val)).await {
                ...
                Err(e) if e.kind() == std::io::ErrorKind::NotFound => {
                    ...
                    warn!("Running task \'{}\' (ID: {}, PID: {}) no longer found. Marking as Failed.", task.name, task.id, pid_val);
                    tasks_to_update.push((task.id, TaskState::Finished, None)); // Store task ID, new state, and None for PID
                }
                Err(e) => {
                    error!("Error checking status for task \'{}\' (ID: {}, PID: {}): {}", task.name, task.id, pid_val, e);
                }
            }
        }
    }
    ...
    for (task_id, new_state, new_pid) in tasks_to_update {
        if let Err(e) = state.update_task_state(task_id, new_state, None).await { // Pass None for assigned_gpus
            error!("Failed to update state for unexpectedly terminated task {}: {}", task_id, e);
        }
        if let Err(e) = state.set_task_pid(task_id, new_pid).await { // Clear PID
