...
fn generate_task_id() -> u64 {
    ...
    let secs_part = SystemTime::now().duration_since(UNIX_EPOCH).unwrap_or_default().as_secs() % 1_000_000;
...
pub async fn handle_submit_command(action: SubmitAction, state: DaemonState) -> Result<Message> {
    match action {
        SubmitAction::Command { command, gpu_num_required, queue_name, name } => {
            ...
            Ok(Message::Ack(format!("Command task {} ('{}') submitted to queue '{}'", task_id, task_name, queue)))
        }
        SubmitAction::Script { script_path, gpu_num_required, queue_name, name } => {
            ...
            Ok(Message::Ack(format!("Script task {} ('{}') submitted from path '{}' to queue '{}'", task_id, task_name, script_path, queue)))
        }
        SubmitAction::BatchJson { mut tasks, default_queue_name } => {
            ...
            for task_meta in tasks.iter_mut() {
                ...
                match state.add_task(task_meta.clone()).await {
                    Ok(_) => {
                        submitted_count += 1;
                        log::info!("Batch task {} ('{}') submitted to queue '{}'", task_meta.id, task_meta.name, task_meta.queue);
                    }
                    Err(e) => {
