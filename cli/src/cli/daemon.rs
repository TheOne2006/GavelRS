...
use colored::*;
use gavel_core::rpc::{
    message::{DaemonAction, Message},
    request_reply,
}; // Import RPC functions and messages
...
impl DaemonCommand {
    ...
    fn handle_start(config: Option<&str>) -> Result<()> {
        ...
        if lock_file_path.exists() {
            ...
            match Self::handle_status(config) {
                Ok(_) => {
                    println!(
                        "{} Daemon appears to be running (RPC status check successful).",
                        "[INFO]".blue()
                    );
                    return Ok(());
                }
                Err(e) => {
                    ...
                    if fs::remove_file(&lock_file_path).is_ok() {
                        ...
                        println!(
                            "{} Failed to remove potentially stale lock file.",
                            "[WARN]".yellow()
                        );
                    }
                }
        ...
        let child = command.spawn().with_context(|| {
            format!("{} Failed to start daemon process: {}", "[ERROR]".red(), daemon_exe.display())
        })?;

        // --- Create lock file with PID ---
        let pid = child.id().to_string();
        fs::write(&lock_file_path, &pid).with_context(|| {
            format!(
                "{} Failed to create or write lock file: {}",
                "[ERROR]".red(),
                lock_file_path.display()
            )
        })?;
        ...
        if let Err(e) = Self::handle_status(config) {
            println!(
                "{} Daemon process started, but initial status check failed: {}",
                "[WARN]".yellow(),
                e
            );
        }
    ...
    fn handle_stop(config: Option<&str>) -> Result<()> {
        ...
        println!(
            "{} Attempting to stop daemon via RPC (socket: {})...",
            "[INFO]".blue(),
            sock_path.cyan()
        ); // Added color
        ...
        match request_reply(&sock_path, &request) {
            Ok(Message::Ack(msg)) => {
                println!(
                    "{} Daemon acknowledged stop request: {}",
                    "[SUCCESS]".green(),
                    msg.italic()
                ); // Added format
                if lock_file_path.exists() {
                    fs::remove_file(&lock_file_path).with_context(|| {
                        format!(
                    ...
                    println!(
                        "{} Lock file removed: {}",
                        "[INFO]".blue(),
                        lock_file_path.display().to_string().cyan()
                    ); // Added color
                } else {
            ...
            Ok(other) => Err(anyhow!(
                "{} Received unexpected reply from daemon during stop: {:?}",
                "[ERROR]".red(),
                other
            )),
            Err(e) => {
                eprintln!(
                    "{} Failed to send stop command or receive reply: {}",
                    "[ERROR]".red(),
                    e
                );
                if lock_file_path.exists() {
                    if fs::remove_file(&lock_file_path).is_ok() {
                        println!(
                            "{} Removed potentially stale lock file: {}",
                            "[INFO]".blue(),
                            lock_file_path.display().to_string().cyan()
                        ); // Added color
                    } else {
                        eprintln!(
                            "{} Failed to remove lock file. Manual cleanup might be needed.",
                            "[WARN]".yellow()
                        );
                    }
                }
                Err(anyhow!("{} Failed to communicate with daemon to stop it.", "[ERROR]".red())
                    .context(e))
            }
        }
    }
    ...
    fn handle_status(config: Option<&str>) -> Result<()> {
        ...
        println!(
            "{} Checking daemon status via RPC (socket: {})...",
            "[INFO]".blue(),
            sock_path.cyan()
        ); // Added color
        ...
        match request_reply(&sock_path, &request) {
            ...
            Ok(Message::Error(err_msg)) => {
                println!(
                    "{} Daemon reported an error status: {}",
                    "[WARN]".yellow(),
                    err_msg.italic()
                ); // Added format
                Ok(())
            ...
            Ok(other) => Err(anyhow!(
                "{} Received unexpected reply from daemon for status request: {:?}",
                "[ERROR]".red(),
                other
            )),
            Err(e) => {
                println!(
                    "{} Daemon is likely not running or unresponsive (RPC failed).",
                    "[WARN]".yellow()
                );
                Err(anyhow!("{} Failed to communicate with daemon for status.", "[ERROR]".red())
                    .context(e))
            }
        }
    }
